...
import { MobileCartDrawer } from "./MobileCartDrawer"; // ⬅️ no hook, no props
import { useAuth } from "../assets/hooks/useAuth";
import { IconUserFilled } from "@tabler/icons-react";
import { useDispatch, useSelector } from "react-redux";
import { openCart } from "../redux/features/cartSlice"; // ⬅️ import action
...
export default function Header() {
  ...
  const handleNavigation = (str?: string) => {
    navigate(`/${str ?? ""}`);
    menu.close(); // ⬅️ close menu drawer after navigation
  };

  useEffect(() => {
    // no-op; kept if you later need side effects
  }, []);
  ...
  return (
    <header className="bg-sandle w-full h-[80px] md:h-[112px] flex items-center px-10 sm:px-12 md:px-14 lg:px-14">
      ...
      {isMobile && (
        <div className="ml-auto flex items-center">
          ...
          <div
            ...
            onClick={() => dispatch(openCart())} // ⬅️ open cart drawer
          >
            {/* cart svg */}
      ...
      <MobileCartDrawer /> {/* ⬅️ redux-controlled drawer; no props */}
